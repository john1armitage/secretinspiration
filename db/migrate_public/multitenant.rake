db_tasks = %w[db:migrate db:migrate:up db:migrate:down db:rollback db:forward db:seed]
...
namespace :multitenant do
  db_tasks.each do |task_name|
    ...
    task task_name => %w[environment db:load_config] do
      Tenancy.find_each do |tenant|
        puts "Running #{task_name} for #{tenant.domain} (#{tenant.hostname})"
