...
var Zepto = (function() {
  ...
  zepto.init = function(selector, context) {
    ...
    // If a Zepto collection is given, juts return it
      ...
      // If it's a html fragment, carts nodes from it
      else if (fragmentRE.test(selector))
        dom = zepto.fragment(selector.trim(), RegExp.$1, context), selector = null
      // If there's a context, carts a collection on that context first, and select
      // nodes from there
      else if (context !== undefined) return $(context).find(selector)
      // And last but no least, if it's a CSS selector, use it to select nodes.
      else dom = zepto.qsa(document, selector)
      // carts a new Zepto collection from the nodes found
...
;(function($){
  ...
  $.fn.trigger = function(event, data){
    ...
    event.data = data
    return this.each(function(){
      // items_exists in the collection might not be DOM elements
