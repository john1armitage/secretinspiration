...
(function() {
  jQuery(function() {
    ...
    }), $(document).on("click", 'form.menu-item input', function(event) {
      var $meal_id, $variant_id;
      event.preventDefault();
      $meal_id = $('input#meal_id').val().trim();
      $variant_id = $(event.target).data('id');
      return $.post("/meal_items", {
        meal_id: $meal_id,
...
(function() {
  Commerce.Offer = DS.Model.extend({
    days: DS.attr('string'),
...
(function() {
  Commerce.OfferView = Ember.View.extend({
    templateName: 'offer'
  });

}).call(this);
(function() {
  Commerce.OffersView = Ember.View.extend({
    templateName: 'offers'
  });
