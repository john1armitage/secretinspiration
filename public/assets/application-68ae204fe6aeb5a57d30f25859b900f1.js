...
(function() {
  jQuery(function() {
    ...
    $(document).on("click", 'form.contact input', function(event) {
      var $contact, $meal_id, $notes, $phone, $start_time;
      ...
      return $.post("/meals/" + $meal_id + "/patcher", {
        takeaway: 'request',
        contact: $contact,
        phone: $phone,
    ...
    $(document).on("click", 'input#bill_payment', function(event) {
      ...
      if (total !== null && total > 0) {
        if (tip > 0) {
          return $('input#order_paid').val(due);
        } else if (total >= due) {
