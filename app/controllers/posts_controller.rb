class PostsController < ApplicationController
  ...
  def index
    ...
    @accountables = Supplier.joins(:posts). where('suppliers.id = postings.accountable_id').order("name").select('name', 'suppliers.id', 'rank').uniq
    #if params[:q]
    ...
    @posts = @q.result(distinct: true).limit(limit).order("event_date #{list_order}, created_at")
