class ApplicationController < ActionController::Base
  ...
  def get_meals
    ...
    @meals = Meal.includes(:line_items, seating: [:booking] ).where("state NOT IN ('billed', 'active', 'complete','main_served') AND state NOT LIKE 'dessert%' AND seating_id::INT > 0").order('priority')
