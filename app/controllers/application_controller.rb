class ApplicationController < ActionController::Base
  ...
  def get_meals
    ...
    @manager_meals = Meal.includes(:line_items, seating: [:booking] ).where("state NOT IN ('billed', 'active', 'complete') AND state NOT LIKE 'dessert%' AND seating_id::INT > 0").order('ordered_at')
